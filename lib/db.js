...
async function migrateStatuses() {
  const sentResult = await DataRecord.updateMany(
    { status: { $in: ['\u0110ang x\u1eed l\u00fd', '\u0110\u00e3 li\u00ean h\u1ec7'] } },
    { $set: { status: '\u0110\u00e3 g\u1eedi tin nh\u1eafn' } }
  );

  const repliedResult = await DataRecord.updateMany(
    { status: { $in: ['\u0110\u00e3 ho\u00e0n th\u00e0nh', 'C\u00f3 ti\u1ec1m n\u0103ng'] } },
    { $set: { status: 'Tr\u1ea3 l\u1eddi' } }
  );
  ...
  const resetResult = await DataRecord.updateMany(
    { status: { $nin: validStatuses, $ne: null, $exists: true } },
    { $set: { status: 'Ch\u01b0a x\u1eed l\u00fd' } }
  );

  if (sentResult.modifiedCount || repliedResult.modifiedCount || resetResult.modifiedCount) {
    console.log('>>> Da cap nhat trang thai du lieu cu sang bo trang thai moi!');
  }
}
